...
/** How long a resolved LNURL `nostrPubkey` is reused. Wallets change rarely. */
const PROVIDER_TTL_MS = 10 * 60 * 1000;
 ...
 *
 * One kind:0 read for all recipients at once, then one HTTPS GET each, both
 * memoised for `PROVIDER_TTL_MS`. Negative results are cached too: a recipient
 * with no `lud16` should not cost an HTTPS round trip on every poll of the
 * gauge.
 */
async function resolveProviders(
  ...
  for (const pubkey of recipients) {
    ...
    if (hit && hit.expiresAt > now) resolved.set(pubkey, hit.nostrPubkey);
